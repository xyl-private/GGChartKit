...
@property (nonatomic, copy) NSString * kLineIndexIndexName;
@property (nonatomic, copy) NSString * volumIndexIndexName;
...
- (void)updateVolumIndexLayer:(NSInteger)index
{
    ...
    runMainThreadWithBlock(^{
        ...
        [weakSelf.volumIndexLayer removeFromSuperlayer];
        
        weakSelf.volumIndexIndexName = [KLineChart kVolumIndexLayerClazzName][index];
        Class clazz = NSClassFromString([weakSelf.volumIndexIndexName stringByAppendingString:@"Layer"]);
        
        weakSelf.volumIndexLayer = [[clazz alloc] init];
        weakSelf.volumIndexLayer.frame = weakSelf.redVolumLayer.frame;
        [weakSelf.volumIndexLayer setKLineArray:weakSelf.kLineArray];
        weakSelf.volumIndexLayer.currentKLineWidth = weakSelf.kLineScaler.shapeWidth;
        [self.scrollView.layer addSublayer:weakSelf.volumIndexLayer];
        [self updateSubLayer];
        
        if (weakSelf.indexChangeBlk) { weakSelf.indexChangeBlk([KLineChart kVolumIndexLayerClazzName][index]); }
    });
}
...
- (void)updateKLineIndexLayer:(NSInteger)index
{
    ...
    runMainThreadWithBlock(^{
        ...
        [weakSelf.kLineIndexLayer removeFromSuperlayer];
        
        weakSelf.kLineIndexIndexName = [KLineChart kLineIndexLayerClazzName][index];
        Class clazz = NSClassFromString([weakSelf.kLineIndexIndexName stringByAppendingString:@"Layer"]);
        
        weakSelf.kLineIndexLayer = [[clazz alloc] init];
        weakSelf.kLineIndexLayer.frame = weakSelf.redLineLayer.frame;
        weakSelf.kLineIndexLayer.gg_width = weakSelf.kLineScaler.contentSize.width;
        [weakSelf.kLineIndexLayer setKLineArray:weakSelf.kLineArray];
        weakSelf.kLineIndexLayer.currentKLineWidth = weakSelf.kLineScaler.shapeWidth;
        [self.scrollView.layer addSublayer:weakSelf.kLineIndexLayer];
        [self updateSubLayer];
        
        if (weakSelf.indexChangeBlk) { weakSelf.indexChangeBlk([KLineChart kLineIndexLayerClazzName][index]); }
    });
}
